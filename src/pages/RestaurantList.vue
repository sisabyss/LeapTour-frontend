  ...
  <div class="container mx-auto pb-4">
    ...
    <div class="px-4 lg:grid lg:grid-cols-12 lg:gap-2">
      ...
      <div class="lg:col-span-9">
        ...
        <div
          v-for="(restaurant, i) in mainContext.restaurants"
          :key="i"
          ...
          <div class="col-span-12 md:col-span-8 px-2 md:px-4 py-2">
            ...
            <p class="flex items-center my-1">
              ...
              <span class="flex items-center mr-1">
                <n-rate readonly :default-value="Number(restaurant?.rating)" />
              </span>

              <!-- Restaurant Reviews Count -->
              <span class="text-sm mr-3"> {{ restaurant?.num_reviews }} Reviews </span>
              ...
              <button
                ...
                :class="restaurant?.open_now_text?.includes('Closed') ? 'bg-red-400' : 'bg-green-400'"
              >
            ...
            <p v-if="restaurant?.cuisine?.length > 0" class="text-xs my-2">
              ...
              <span v-for="(item, i) in restaurant?.cuisine" :key="i" class="bg-gray-300 rounded-full inline-block px-2 py-1 mr-1 mb-1">
                {{ item?.name }}
              </span>
            ...
            <div v-if="restaurant?.reviews && restaurant?.reviews[0] !== null && restaurant?.reviews?.length > 0">
              <div v-for="(review, i) in restaurant?.reviews" :key="i">
...
watch(
  ...
  () => {
    ...
    getPlacesByLatLng('restaurants', mainContext.coordinates.lat, mainContext.coordinates.lng, { limit: 30, min_rating: 4 }, source).then((data) => {
      ...
      mainContext.restaurants = data?.filter((restaurant) => restaurant.name);
