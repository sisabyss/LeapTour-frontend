...
<template>
  <div class="flex items-center w-full p-2 absolute top-0 z-10">
    <div class="flex bg-white justify-between items-center w-full p-3 md:p-4 rounded-sm shadow-md">
      <!-- Logo displays only when  -->
      <img v-if="search" src="../../assets/logo.svg" alt="TravelAdvisor" class="h-6 sm:h-7 md:h-8" />

      <!-- Search Form - Toggle between Hidden and Visible on Mobile, determined by the 'search' state -->
      <div class="relative w-full md:w-auto md:block" :class="{ hidden: search }">
      ...
      <div class="cursor-pointer md:hidden p-2 -mr-2">
        <!-- Search Button - Displays when search = false -->
        <svg
          v-if="!search"
          class="w-5 h-5 transition ease-out duration-1000"
          xmlns="http://www.w3.org/2000/svg"
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor"
